...
mal_eval(list([symbol('do') | Exprs])) -->
    mal_eval(Exprs).
mal_eval(list([symbol('def!'), symbol(Name), Expr])) -->
    mal_eval(Expr),
    result(Val),
    global(Env0, Env),
    { put_assoc(Name, Env0, Val, Env) }.
...
mal_eval(list([symbol('quote'), Expr])) -->
    result(Expr).
mal_eval(list([symbol('quasiquote'), Expr])) -->
    quasiquote(Expr),
    result([Result], Result).
...
namespace(NS) :-
    ...
    open_string("(def! load-file (fn* (f) (eval (read-string (str \"(do \" (slurp f) \")\")))))",
                Load),
