...
int gen_do_code(MalType *list, MalEnv *env, struct codegen *code, int ret, int *var_num) {
  ...
  for (iter = list_or_vector_iter(list); iter; iter = list_or_vector_iter_next(iter)) {
    ...
    if (!gen_code(node, env, &code2, ret && is_last, var_num)) {
      return 0;
    }
    if (is_last) {
...
int gen_if_code(MalType *node, MalEnv *env, struct codegen *code, int ret, int *var_num) {
  ...
  if (is_primitive(true_val)) {
    ...
    MalType *fn_name = next_var_name("if_true", var_num);
    if (!gen_fn_code(fn_name, mal_cons(mal_empty(), mal_cons(true_val, mal_empty())), env, &code2, var_num, 1)) {
      return 0;
    }
    mal_string_append_mal_string(true_code, mal_sprintf("%S(%S, 0, NULL)", fn_name, build_env_name(env)));
  }
  node = mal_cdr(node);
  ...
  if (is_primitive(false_val)) {
    ...
    MalType *fn_name = next_var_name("if_false", var_num);
    if (!gen_fn_code(fn_name, mal_cons(mal_empty(), mal_cons(false_val, mal_empty())), env, &code2, var_num, 1)) {
      return 0;
    }
    mal_string_append_mal_string(false_code, mal_sprintf("%S(%S, 0, NULL)", fn_name, build_env_name(env)));
  }
  append_code(
    code->body,
