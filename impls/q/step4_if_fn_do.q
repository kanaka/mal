...
do_do:{[partial_ast; env];
  last first accumulate[notempty; partial_ast; {[asts; env]; (EVAL[first asts; env]; tail asts)}[;env]]};

do_if:{[partial_ast; env];
  cty:first EVAL[first partial_ast; env];
  $[not ((cty = `false) or (cty = `nil)); EVAL[partial_ast @ 1; env];
  $[2 = count partial_ast; (`nil; ()); EVAL[partial_ast @ 2; env]]]};
